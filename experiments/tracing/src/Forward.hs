...
module Forward (forward, purge, getName, Forward, Forwarded (FLift, FOp0, FOp1, FOp2, FMap, FMapV, FFold, FFoldV), FValue (FArray, FBool, FReal, FFunc)) where
            ...
            controller t s c' fs vs
                | mod (len - s) t == 0 =
                    ...
                    then let size = len `div` vectorizedFoldThreads
                             sarr = take (size - 1) xs
                             (vf, ff, cf) = forwardFoldFull fn c' sa z sarr 1 f
                             ff' = Map.singleton (getName vf) (FFold ff sa (getName z), 1, vf)
                         in  controller (t - 1) size cf ff' [unliftFloat vf]
                         ...
                         in  controller (t - 1) (s + size) cf ff' (unliftFloat vf : vs)
                | otherwise            =
                    if   s == 0
    ...
    purge f s = 
        ...
        where
            ...
            check ks k   (FMap   fs a,      c, v) = (k : ks, (FMap (purgeMap fs 0) a, c, v))
                where
